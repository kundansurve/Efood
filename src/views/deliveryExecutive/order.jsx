...
function Order(props) {
    ...
    const apiCall = () => {
        fetch(`${window.location.origin}/api/delivery-executive/me/order/${orderId}`)
            .then(resp => resp.json())
            .then(data => {
                setOrderDetails(data.order);
                fetch(`${window.location.origin}/api/users/user/${data.order.placedByUserId}`)
                    .then(resp => resp.json())
                    .then((deliverTo) => {
                        setDeliverTo(deliverTo);
                    }).catch(error => console.log(error))
                fetch(`${window.location.origin}/api/hotels/hotel/${data.order.placedInHotelId}`)
                    .then(resp => resp.json())
                    .then((hotelData) => {
                        setOrderFromHotel(hotelData.hotel);
                    }).catch(error => console.log(error))
                    fetch(`${window.location.origin}/api/cities/city/${data.order.cityId}`)
                    .then(resp => resp.json())
                    .then((CityDetails) => {
                ...
                fetch(`${window.location.origin}/api/hotel/dishes/${data.order.placedInHotelId}`)
                    .then(resp => resp.json())
                    .then((dishes) => {
    ...
    const recievedFromHotel = () => {
        ...
        fetch(`${window.location.origin}/api/delivery-executive/me/order/recieved-from-hotel`, {
            method: "PUT",
    ...
    const delivered = () => {
        ...
        fetch(`${window.location.origin}/api/delivery-executive/me/order/delivered`, {
            method: "PUT",
            headers: {
                "Content-type": "application/json; charset=UTF-8",
            },
