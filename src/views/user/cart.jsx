...
function Cart(props){
        ...
        useEffect(()=>{
            ...
            fetch(
                "http://localhost:4000/api/hotel/" +hotel
              )
        ...
        return (
            (state.openModal)?<>
        <SetAddress close={()=>{setState({...state,openModal:false})}} onclick={(address)=>{setState({...state,address:address})}}/></>
        :<div style={{paddingBottom:"4em"}}>
            ...
            <div style={{width:"50%", minWidth:"360px",alignItems:"center",padding:"1em",margin:"1em"}}>
                <div style={{padding:"1em", margin:"1em",backgroundColor:"#efefef",borderRadius:"5px",width:"90%"}}>
                    <h5>Order From</h5>
                    {(!hotel)?<>None</>:<><p style={{padding:"2px",marginBottom:"0px"}}>{hotelData.name}</p>
                    <span style={{alignItems:"center",display:"flex"}}><img src="https://img.icons8.com/office/16/000000/marker.png"/><p style={{margin:"0px"}}>{hotelData.cityId}</p></span></>}
                </div>
                <div style={{width:"90%"}}>
            ...
            <div style={{width:"90%",padding:"1em",border:"2px solid #efefef",borderRadius:"5px",margin:"1em"}}>
                ...
                <button style={{margin:"auto auto",background:"transparent",border:"none", textDecoration:"underline", textAlign:"center"}} onClick={()=>{setState({...state,openModal:true})}}>Change Address</button>
            </div>
